...
});};var f={};h.Loader.loadModule=function(i,j){i=i.replace(/^(?:Briage\.)?(.*?)(?:\.js)?$/,"Briage.$1.js");if(f[i]){if(j){f[i].handles.push(j);}}else{f[i]={onload:function(){h.each(f[i].handles,function(m,l){l();
});delete f[i];},handles:[]};if(j){f[i].handles.push(j);}h.Loader.loadScript(i,i,function(k){if(k==h.Loader.State.HAS_BEEN_LOADED){f[i].onload();}});}};
h.Loader.loadModules=function(j,k){var i=0;var l=0;h.each(j,function(){i++;});if(i==0){if(k){k();}}h.each(j,function(n,m){h.Loader.loadModule(m,function(){l++;
if(i==l){if(k){k();}}});});};var e=new h.Class(function(){if(!(this instanceof e)){return new e();}return this;},{prototype:{use:function(){var j=arguments.length;
if(!j){return;}var i;var k;if(h.toString.call(arguments[0])=="[object Array]"){i=arguments[0];if(j==1){k=null;}else{if(typeof arguments[1]=="function"){k=arguments[1];
}else{k=null;}}}else{if(typeof arguments[j-1]=="function"){i=Array.prototype.slice.call(arguments,0,-1);k=arguments[j-1];}else{i=arguments;k=null;}}h.Loader.loadModules(i,function(){if(k){k(h.deepclone(h,true));
}});},add:function(l,j,i,k){j=j.replace(/^(?:Briage\.)?(.*?)(?:\.js)?$/,"Briage.$1.js");k=k||{};this.use(i,function(){h.Loader.loadScripts(k.scripts,function(){var o=new e.Event(j);
