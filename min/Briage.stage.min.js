...
}});}}return false;};a.Stage.Vector=new a.Class(function(b,c){this.x=b;this.y=c;},{method:{paese:function(){var c=arguments[0];var b=arguments[1];if(c instanceof a.Stage.Vector){return c.clone();
}var d=a.toString.call(c);if(d=="[object Array]"){return new this(c[0],c[1]);}if(d=="[object Object]"){return new this(c.x,c.y);}return new this(arg,b);
}},prototype:{x:0,y:0,equals:function(b){return b&&this.x==b.x&&this.y==b.y;},add:function(b){if(b){this.x=this.x+b.x;this.y=this.y+b.y;}},scale:function(b){this.x=this.x*b;
this.y=this.y*b;},clone:function(){return new a.Stage.Vector(this.x,this.y);}}});a.Stage.Timer=a.Timer;a.Stage.Event=new a.Class(function(){},{prototype:{returnValue:true,cancelBubble:false,preventDefault:function(b){this.returnValue=false;
